...
@Component({
  selector: 'app-addnewpt',
  templateUrl: './addnewpt.component.html',
  styleUrls: ['./addnewpt.component.scss']
})
export class AddnewptComponent implements OnInit {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<AddnewptComponent>,
  ...
  listColumnName: any = [];
  listReportColumn: any = ['ROW NUMBER'];
  listPT_type: any = [];
  report_name: any;
  selected: any;
  ...
  onSubmit() {
    ...
    for (let i = this.listReportColumn.length - 1; i >= 0; i--) {
      ...
      sq = sq + "'" + this.report_name + "','" + this.listReportColumn[i] + "','" + pt_tp + "'" + '\n'
      values.push([sq]);
    }

    let rawdata = this.formData.value;
    this.ptService.deletReportName(this.report_name).subscribe(
      (res: any) => {
        // Handle delete response if needed
      } 
    );

    this.ptService.addReportName(sq).subscribe(
      (res: any) => {
